...
do
	...
	local function Pickup(actionType, actionData)
		...
		elseif actionType == "currency" then
			return C_CurrencyInfo.PickupCurrency(actionData)
	...
	local function ParseActionInfo(actionType, data1, data2)
		...
		elseif actionType == "currency" then
			return true, "currency", (C_CurrencyInfo.GetCurrencyInfo(data1))
		elseif actionType == "macro" then
			return true, "macro", (GetMacroInfo(data1))
		elseif actionType == "equipmentset" then
			return true, "equipmentset", (C_EquipmentSet.GetEquipmentSetInfo(data1))
...
do
	local function AcceptActionType(self, actionType)
		return actionType == "item" or actionType == "currency" or actionType == "spell" or actionType == "macro" or actionType == "equipmentset"
	end	
	...
	local function ParseValue(self, value)
		...
		local currencyId = tonumber(value:match("currency:(%d+)"))
		if currencyId then
			local currencyInfo = C_CurrencyInfo.GetBasicCurrencyInfo(currencyId)
			local name = currencyInfo.name
			local texture = currencyInfo.icon
			return "currency", currencyId, name, texture
		end
		...
		if setName then
			...
			for i = 1, GetNumEquipmentSets() do
				local name, texture = C_EquipmentSet.GetEquipmentSetInfo(i)
...
do
	...
	local function ParseValue(self, value)
		local currencyId = tonumber(value) or tonumber(value:match("currency:(%d+)"))
		if currencyId then
			local name, _, _, _, _, _, texture = C_CurrencyInfo.GetCurrencyInfo(currencyId)
			return "currency", currencyId, name, texture
		end
	end
	...
	local widgetType = "ActionSlotCurrency"
